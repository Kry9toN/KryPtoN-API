...
def ytv():
	if request.args.get('url'):
		try:
			...
			dl_link = bs(post('https://www.y2mate.com/mates/convert',data={'type':'youtube','_id':id[1],'v_id':url.split('/')[3],'ajax':'1','token':'','ftype':'mp4','fquality':'360p'}).json()['result'],'html.parser').find('a')['href']
			return {
				'status': 200,
				'title': title,
				'thumb': thumb,
...
def yta():
	if request.args.get('url'):
		try:
			...
			dl_link = bs(post('https://www.y2mate.com/mates/convert',data={'type':'youtube','_id':id[1],'v_id':url.split('/')[3],'ajax':'1','token':'','ftype':'mp3','fquality':'128'}).json()['result'],'html.parser').find('a')['href']
			return {
				'status': 200,
				'title': title,
				'thumb': thumb,
